...
public class InstantCalibration : MonoBehaviour
{
    private static readonly float CentimetersToMeters = 0.01f;
    private static readonly float InchesToMeters = 0.0254f;
    private Vector3 calibratedCenterPosition;
    private Quaternion calibratedCenterRotation;
    private float calibratedFloorPosition; //updateCAnonStorey2.controller1Transform.position.y - 0.05f;
    private float heightOffset;
    private readonly bool heightOffsetInInchesNotCentimeters = false;


    private void Update()
    {
        if (Input.GetKeyUp(KeyCode.C))
            StartCoroutine(CalibrateAfterFrame());
    }

    private IEnumerator CalibrateAfterFrame()
    {
        SteamVR_MonitorUtil.DeleteChaperone_Info();
        OpenVR.ChaperoneSetup.ReloadFromDisk(EChaperoneConfigFile.Live);
        yield return null;
        var deviceIdx = GetComponent<SteamVR_TrackedObject>().index;
    ...
    private void Calibrate(Vector3 calibratedCenterPosition, Quaternion calibratedCenterRotation,
    ...
    private void GetStandingOnlyConfigurationValues(out Vector3 zeroPosePosition, out Quaternion zeroPoseRotation,
